"""Evaluation and benchmarking CLI (``locitorium eval ...``).

These commands read a gold ``dataset.jsonl`` and write ``predictions.jsonl``,
or compare predictions against gold. For pipeline use see
``locitorium resolve`` in :mod:`locitorium.cli`.
"""
...
from locitorium.eval.io import load_gold, load_predictions, read_jsonl
from locitorium.eval.metrics import topk_accuracy
from locitorium.pipeline.runner import run_dataset_stream

app = typer.Typer(
    add_completion=False,
    help="Evaluation and benchmarking commands (dataset.jsonl in, metrics out).",
)
